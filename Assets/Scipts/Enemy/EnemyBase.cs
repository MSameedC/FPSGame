...
public abstract class EnemyBase : MonoBehaviour, IMoveable, IDamageable, IKnockback, IPoolable
{
    ...
    public void OnSpawn()
    {
        // Randomize attackTimer for random enemy start time
    ...
    // public bool HasRecentPlayerSight()
    // {
    //     float lastSightTimeFrame = 3;
    //     return Time.time - lastSightTime < lastSightTimeFrame;
    // }
    ...
    public virtual void TakeDamage(int amount)
    {
        ...
        if (CurrentHealth <= 0)
            SetState(new EnemyDeadState(this));
    }
